...
int main(int argc, char *argv[])
{
	...
	int first_byte = 0, second_byte = 0;
		...
		if(first_byte == 0x0d){//collect data of the form (i, 13, k) in histograms[0][i][k]
			 histograms[0][i & MASK_8][second_byte]++; //increment the relevant histogram count
		} else if (first_byte == 0xf0) { //(i, 240, k) -> histograms[1][i][k]
			 histograms[1][i & MASK_8][second_byte]++; //increment the relevant histogram count
		} else if (first_byte == 0x81) { //(i, 129, k) -> histograms[3][i][k]
			 histograms[2][i & MASK_8][second_byte]++;
		} else if (second_byte == 0xf0) { //(i, j, 240) -> histograms[2][i][j]
			 histograms[3][i & MASK_8][first_byte]++; //increment the relevant histogram count
		}
...
void dumpDoubleByteData(std::ostream &outStream, uint64_t histograms[][RC4Stream::PERMUTATION_ARRAY_LENGTH][RC4Stream::PERMUTATION_ARRAY_LENGTH], uint32_t numberOfBatches, int proc_id){
	    ...
	    //First Histogram
	    outStream << "% The entry histogrami13k[i][k] is the count of occurences of (i, 13, k)  " << endl
            	      << "histogram_i13k_id" << proc_id << " = [ " << endl;
	    ...
	    //Second Histogram
	    outStream << endl << "% The entry histogrami240k[i][k] is the count of occurences of (i, 0xf0, k)  " << endl
            	      << "histogram_i240k_id" << proc_id << " = [ " << endl;
            for(int i = 0; i < RC4Stream::PERMUTATION_ARRAY_LENGTH; i++) {
                outStream << '\t';
                for(int k = 0; k < RC4Stream::PERMUTATION_ARRAY_LENGTH; k++){
	    ...
	    //Third Histogram
	    outStream << endl << "% The entry histogrami129k[i][k] is the count of occurences of (i, 129, k)" << endl
            	      << "histogram_i129k_id" << proc_id << " = [ " << endl;
            for(int j = 0; j < RC4Stream::PERMUTATION_ARRAY_LENGTH; j++) {
                outStream << '\t';
                for(int k = 0; k < RC4Stream::PERMUTATION_ARRAY_LENGTH; k++){
                    outStream << histograms[2][j][k] << ' ';   
                }
                outStream << ';' << endl;
            }
            outStream << "\t];" << endl;
	    ...
	    outStream << endl << "% The entry histogramij240[i][j] is the count of occurences of (i, j, 0xf0)  " << endl
            	      << "histogram_ij240_id" << proc_id << " = [ " << endl;
            for(int i = 0; i < RC4Stream::PERMUTATION_ARRAY_LENGTH; i++) {
                outStream << '\t';
                for(int j = 0; j < RC4Stream::PERMUTATION_ARRAY_LENGTH; j++){
                    outStream << histograms[3][i][j] << ' ';   
                }
                outStream << ';' << endl;
            }
            outStream << "\t];" << endl;
	    ...
	    //Record the number of bytes collected and calculate the experimental conditional probabilities
            outStream << endl << "% The total Number of bytes of data collected for each index i" << endl 
			      ...
			      //Ensure histogramij254 is complete
		      << endl << "% Ensure that the histograms contain all required data by supplementing them from each other where necessary" << endl
			      << "% Supplement histogram_ij250_id" << proc_id << " from the last but 15 columns of i13k, i250k, and i129k" << endl
			      << "histogram_ij240_id" << proc_id << "(:, 14) = histogram_i13k_id" << proc_id << "(:, 241);" << endl
			      << "histogram_ij240_id" << proc_id << "(:, 251) = histogram_i240k_id" << proc_id << "(:, 241);" << endl
			      << "histogram_ij240_id" << proc_id << "(:, 130) = histogram_i129k_id" << proc_id << "(:, 241);" << endl << endl

             	  << "% Set the format to longe to capture the differences in the experimental probabilities" << endl
                  << "format longe" << endl;
