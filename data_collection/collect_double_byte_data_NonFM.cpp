...
int main(int argc, char *argv[])
{
	...
	int first_byte = 0, second_byte = 0;
		...
		if(first_byte == 0x01){//collect data of the form (i, 1, k) in histograms[0][i][k]
			 histograms[0][i & MASK_8][second_byte]++; //increment the relevant histogram count
		} else if (first_byte == 0xfe) { //(i, 254, k) -> histograms[1][i][k]
			 histograms[1][i & MASK_8][second_byte]++; //increment the relevant histogram count
		} else if (first_byte == 0x80) { //(i, 128, k) -> histograms[3][i][k]
			 histograms[2][i & MASK_8][second_byte]++;
		} else if (second_byte == 0xfe) { //(i, j, 254) -> histograms[2][i][j]
...
void dumpDoubleByteData(std::ostream &outStream, uint64_t histograms[][RC4Stream::PERMUTATION_ARRAY_LENGTH][RC4Stream::PERMUTATION_ARRAY_LENGTH], uint32_t numberOfBatches, int proc_id){
	    ...
	    //First Histogram
	    outStream << "% The entry histogrami1k[i][k] is the count of occurences of (i, 0x01, k)  " << endl
            	      << "histogram_i1k_id" << proc_id << " = [ " << endl;
	    ...
	    //Second Histogram
	    outStream << endl << "% The entry histogrami254k[i][k] is the count of occurences of (i, 0xfe, k)  " << endl
            	      << "histogram_i254k_id" << proc_id << " = [ " << endl;
            for(int i = 0; i < RC4Stream::PERMUTATION_ARRAY_LENGTH; i++) {
                outStream << '\t';
                for(int k = 0; k < RC4Stream::PERMUTATION_ARRAY_LENGTH; k++){
	    ...
	    outStream << endl << "% The entry histogramij254[i][j] is the count of occurences of (i, j, 0xfe)  " << endl
            	      << "histogram_ij254_id" << proc_id << " = [ " << endl;
            for(int i = 0; i < RC4Stream::PERMUTATION_ARRAY_LENGTH; i++) {
                outStream << '\t';
                for(int j = 0; j < RC4Stream::PERMUTATION_ARRAY_LENGTH; j++){
	    ...
	    //Record the number of bytes collected and calculate the experimental conditional probabilities
            outStream << endl << "% The total Number of bytes of data collected for each index i" << endl 
		      ...
		      << endl << "% Ensure that the histograms contain all required data by supplementing them from each other where necessary" << endl
			      << "% Supplement histogram_ij254_id" << proc_id << " from the last but one columns of i1k, i254k, and i128k" << endl
			      << "histogram_ij254_id" << proc_id << "(:, 2) = histogram_i1k_id" << proc_id << "(:, 255);" << endl
			      << "histogram_ij254_id" << proc_id << "(:, 255) = histogram_i254k_id" << proc_id << "(:, 255);" << endl
			      << "histogram_ij254_id" << proc_id << "(:, 129) = histogram_i128k_id" << proc_id << "(:, 255);" << endl << endl
