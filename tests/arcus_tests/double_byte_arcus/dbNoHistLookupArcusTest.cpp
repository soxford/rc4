 ...
 * Description: A test of the doublebyte collection code with no Histogram lookups, written for arcus-b
 * License: 
 * Date: June 2015
 * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
...
const char* TEST_NAME = "Double Byte No Histogram Lookup";
...
int main(int argc, char *argv[])
{
  ...
  string fileName = "ArcusTestData" + to_string((long long) my_proc_id) + ".txt";
   ...
   for (int bytecount_power = 16 ; bytecount_power <= MAX_BYTE_COUNT_POWER; bytecount_power += 3 ) {
	 ...
	 //TEST no Histogram lookup histograms[i & MASK_8][first_byte][second_byte]++; //increment the relevant histogram count
         i&MASK_8;
