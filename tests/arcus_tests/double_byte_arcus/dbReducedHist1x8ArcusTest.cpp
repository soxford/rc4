 ...
 * Description: A test of the doublebyte collection code using a 1x256x256 histogram to check cache improvements
 * License: 
 * Date: June 2015
 * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
...
#define HISTOGRAM_J_COUNT 8
#define MASK_3 0x07
//variables used in documenting the test
const char* TEST_NAME = "Double Byte Test with histogram i index size reduced to 1 and j index size 8";
...
//allocate space to hold table of results TEST 1x256x256
long histograms[HISTOGRAM_I_COUNT][HISTOGRAM_J_COUNT][RC4Stream::PERMUTATION_ARRAY_LENGTH]; 
using namespace std;
...
int main(int argc, char *argv[])
{
  ...
  string fileName = "ArcusTestData" + to_string((long long) my_proc_id) + ".txt";
   ...
   for (int i = 0; i < HISTOGRAM_I_COUNT; i++) {
      for (int j = 0; j < HISTOGRAM_J_COUNT; j++) {
   ...
   for (int bytecount_power = 16 ; bytecount_power <= MAX_BYTE_COUNT_POWER; bytecount_power += 3 ) {
      ...
      first_byte = rc4Stream.PRGRound() & MASK_3;
      
      for (int i = 0; i < bytecount; i++) {
         second_byte = rc4Stream.PRGRound();
         //TEST use 0 from i index on all rounds and first byte is mod 8
         //TEST histograms[i & MASK_8][first_byte][second_byte]++; //increment the relevant histogram count
         histograms[0][first_byte][second_byte]++; //increment the relevant histogram count
         first_byte = second_byte & MASK_3;
      }
