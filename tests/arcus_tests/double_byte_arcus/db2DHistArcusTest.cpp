...
const char* TEST_NAME = "Double Byte Test with histogram no i index and j index size 8";
...
//allocate space to hold table of results TEST 8x256
long histograms[HISTOGRAM_J_COUNT][RC4Stream::PERMUTATION_ARRAY_LENGTH]; 
using namespace std;
  ...
  string fileName = "ArcusTestData" + to_string((long long) my_proc_id) + ".txt";
   ...
   for (int bytecount_power = 16 ; bytecount_power <= MAX_BYTE_COUNT_POWER; bytecount_power += 3 ) {
      ...
      for (int i = 0; i < bytecount; i++) {
         ...
         histograms[first_byte][second_byte]++; //increment the relevant histogram count
