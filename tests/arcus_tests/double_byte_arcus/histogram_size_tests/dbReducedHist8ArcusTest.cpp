 ...
 * Description: A test of the doublebyte collection code using a 8x256x256 histogram to check cache improvements
 * License: 
 * Date: August 2015
 * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
...
#define MASK_3 0x07
#define HISTOGRAM_I_COUNT 8
//variables used in documenting the test
const char* TEST_NAME = "Double Byte Test with histogram i index size reduced to 8";
  ...
  string fileName = "ArcusTestData" + to_string((long long) my_proc_id) + ".txt";
   ...
   for (int bytecount_power = 16 ; bytecount_power <= MAX_BYTE_COUNT_POWER; bytecount_power += 3 ) {
      ...
      for (int i = 0; i < bytecount; i++) {
         ...
         histograms[i & MASK_3][first_byte][second_byte]++; //increment the relevant histogram count
