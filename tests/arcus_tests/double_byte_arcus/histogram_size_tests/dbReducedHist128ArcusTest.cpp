 ...
 * Description: A test of the doublebyte collection code using a 128x256x256 histogram to check cache improvements
 * License: 
 * Date: August 2015
 * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
...
#define MASK_7 0x7f
#define HISTOGRAM_I_COUNT 128
//variables used in documenting the test
const char* TEST_NAME = "Double Byte Test with histogram i index size reduced to 128";
  ...
  string fileName = "ArcusTestData" + to_string((long long) my_proc_id) + ".txt";
   ...
   for (int bytecount_power = 16 ; bytecount_power <= MAX_BYTE_COUNT_POWER; bytecount_power += 3 ) {
      ...
      for (int i = 0; i < bytecount; i++) {
         ...
         //TEST use MASK_7 instead of MASK_8 as the i index has size 4
         //TEST histograms[i & MASK_8][first_byte][second_byte]++; //increment the relevant histogram count
         histograms[i & MASK_7][first_byte][second_byte]++; //increment the relevant histogram count
