 ...
 * Description: A control test of the doublebyte collection code with no rc4 rounds, written for arcus-b
 * License: 
 * Date: June 2015
 * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
...
const char* TEST_NAME = "Double Byte No RC4 Generation Test";
  ...
  string fileName = "ArcusTestData" + to_string((long long) my_proc_id) + ".txt";
   ...
   for (int bytecount_power = 16 ; bytecount_power <= MAX_BYTE_COUNT_POWER; bytecount_power += 3 ) {
      ...
      for (int i = 0; i < bytecount; i++) {
         second_byte = i & MASK_8; //TEST rc4Stream.PRGRound();
         //TODO should i & MASK_8 be calculated separately and should i be used or RC4Stream::_i - 1 ?
         histograms[second_byte][first_byte][second_byte]++; //increment the relevant histogram count
