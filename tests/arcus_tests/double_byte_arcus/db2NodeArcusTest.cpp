 ...
 * Description: A test of the doublebyte collection code using a 4x256x256 histogram to check cache improvements whilst selecting what data to store
 * License: 
 * Date: June 2015
 * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
...
const char* TEST_NAME = "Double Byte Test with histogram i index size reduced to 4 and data selectively stored, to be run on 2 nodes";
...
int main(int argc, char *argv[])
{
  ...
  string fileName = "2NodeArcusTestData" + to_string((long long) my_proc_id) + ".txt";
   ...
   for (int bytecount_power = 16 ; bytecount_power <= MAX_BYTE_COUNT_POWER; bytecount_power += 3 ) {
      ...
      for (int i = 0; i < bytecount; i++) {
         ...
         //TEST map indices to histogram selectively
