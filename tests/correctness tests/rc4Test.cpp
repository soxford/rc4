...
int main(int argc, const char *argv[])
{
   ...
   uint8_t shortTargets[3][RC4Stream::Key::KEY_LENGTH] = 
   ...
   //Target first 16 btes of output generated by the openssl rc4 command line tool for the keys above
   //TODO get test data for this array uint8_t longTargets[3][257] = 
                        {  { 0xe9, 0x9c, 0x40, 0xf9, 0x47, 0xe2, 0x19, 0xcc, 0x06, 0xdb, 0x97, 0xc6, 0x0e, 0xdd, 0x2a, 0x4f },
                           { 0x0b, 0x9c, 0x1a, 0xec, 0x15, 0x0e, 0x45, 0xa8, 0xa7, 0x43, 0xe7, 0xc1, 0xe7, 0x4a, 0x1f, 0x9b },
                           { 0x08, 0x46, 0x71, 0xc8, 0x83, 0x5e, 0xe8, 0xf9, 0x09, 0xa8, 0xb4, 0x8f, 0x7d, 0xd2, 0x15, 0xb3 } };
   ...
   uint8_t output[RC4Stream::Key::KEY_LENGTH]; //store output bytes
   ...
   for (int i = 0; i < 3; i++) {
     ...
     rc4Stream.PRGOutputFirst16BytesToArray(output);
      
     //run RC4 stream algorithm and collect output in histogram counters
     for (int j = 0; j < RC4Stream::Key::KEY_LENGTH; j++) {
        if(shortTargets[i][j] != output[j]){
            cout << "missmatch from short target " << i + 1 << " in byte no. " << j + 1 << endl;
            return 1;
        }
     }
     //re-schedule key
     ...
     for (int j = 0; j < 257; j++) {
        if(longTargets[i][j] != output[j]){
            cout << "missmatch from long target " << i + 1 << " in byte no. " << j + 1 << endl;
            return 1;
        }
     }
