%Code to generate 15000 synthetic random samples for each ciphertext count from 2^{8+1} up to 
%2^{8+samplecount}. This data is then used to measure the average number of
%guesses to get the correct message byte using the "max-peak" attack. 
%The inputs are the a matrix of distributions for each byte position under 
%consideration, which are taken to be the correct RC4 output distributions,
%and a samplecount, which indicates the number of different ciphertext counts to be
%considered. 
...
%base power for each sample size (ciphertext counts of less than 2^8 are
...
for i = 1:samplecount
    ...
    %Count how many guesses would be required to find the correct value of 
    %m_r (which is 0 in this case) by searching through the peaks in order
    %(assume that if multiple candidates have the same value then all of theme 
    % are tried to give a worst case analysis)
